...
The task examples that are obtained from commercial LLM are as follows: Acquisition (dose reduction, scan time reduction), Analysis (diagnosis, education, prognosis prediction and assessment, and treatment planning), Reconstructions, Enhancement (attenuation correction, denoising, motion correction, scatter correction, and super-resolution), Registration, Synthesis (cross-modality synthesis, data augmentation), Quantification (dosimetry, kinetic modeling, metabolic tumor volume (MTV), SUV estimation, total lesion glycolysis (TLG)). The tasks can be implemented via AI-based methods, such as classification, detection, generation, and segmentation. <br>
